...
cell_types <- unique(blood_sc$predicted.celltype.l2)
...
for(combo in pat_cell_combos){
  ...
  which <- which(blood_sc$UCD == pat & blood_sc$predicted.celltype.l2 == cell)
...
for(assay in c("RNA","ATAC","ADT")){
  norm_mat <- feature_by_pat_cell_combos[[assay]]
  if(assay == "RNA"){
    ...
    transcript_lengths_df <- data.frame(gene = rownames(norm_mat), transcript_length = NA)
    ...
    which <- which(!is.na(transcript_lengths_df$transcript_length ))
    norm_mat <- norm_mat[which,] 
    transcript_lengths_df <- transcript_lengths_df[which,]
    
    
    ## perform normalization with using acquired transcript lengths
    
    norm_mat ## TPM = 10e6 * (mat[el,col] / transcript_length[el]) / sum(mat[,col]/transcript_length #matched order)
